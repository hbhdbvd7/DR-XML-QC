...
def _missing_media_description_cn(match: MediaMatch) -> str:
    if match.clip.decoded_path or match.clip.pathurl:
        return f"XML 第 {match.clip.index} 个片段 `{match.clip.name}` 对应的文件名没有在本地素材目录中找到。"
...
def _ignore_empty_path_missing_match(match: MediaMatch, comparison: ClipComparison | None) -> bool:
    ...
    if comparison is None:
        return False
    if not _normalize_name(match.clip.name) or _normalize_name(match.clip.name) != _normalize_name(comparison.timeline_name):
        return False
    return comparison.track_index == (match.clip.track_index or comparison.track_index)
