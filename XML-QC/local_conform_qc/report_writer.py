...
def build_failure_report(message: str, context: dict[str, Any] | None = None) -> ReportData:
    """Create a report payload for failures that happen before QC completes."""
    return ReportData(
        status="error",
...
def _render_key_value_section(title: str, values: Any) -> str:
    ...
    rows = "\n".join(
        f"<tr><th>{escape(str(key))}</th><td>{escape(_short_text(value))}</td></tr>"
...
def _render_issues(issues: Any) -> str:
    if not issues:
        return "<section><h2>问题列表</h2><p>没有记录问题。</p></section>"
    if not isinstance(issues, list):
        return _render_key_value_section("问题列表", issues)
    rows = []
    for issue in issues:
        if not isinstance(issue, dict):
            rows.append(f"<tr><td colspan=\"4\">{escape(_short_text(issue))}</td></tr>")
            continue
        rows.append(
            "<tr>"
            ...
            f"<td><pre>{escape(json.dumps(issue.get('context', {}), ensure_ascii=False, indent=2))}</pre></td>"
            "</tr>"
        )
    return (
        "<section><h2>问题列表</h2>"
        "<table><thead><tr><th>严重程度</th><th>代码</th><th>说明</th><th>上下文</th></tr></thead>"
...
def _render_sections(sections: Any) -> str:
    ...
    for name, value in _ordered_sections(sections):
        if isinstance(value, dict):
            ...
            rendered.append(
                f"<section><h2>{escape(str(name))}</h2><pre>{escape(json.dumps(value, ensure_ascii=False, indent=2))}</pre></section>"
            )
    return "\n".join(rendered)
...
def _ordered_sections(sections: dict[str, Any]) -> list[tuple[str, Any]]:
    ...
    for name in priority:
        if name in sections:
            ...
            used.add(name)
    ranked.extend((name, value) for name, value in items if name not in used)
    return ranked
...
def _render_dict_list_section(title: str, values: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        f"<table><thead><tr>{header}</tr></thead><tbody>{''.join(body_rows)}</tbody></table></section>"
    )
